    ...
    <div className="bg-white border border-[#E4ECE9] border-dashed rounded-xl p-8 shadow-sm h-full flex flex-col">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[#475569] text-[10px] opacity-60 cursor-help" aria-label="Information tooltip">(i)</span>
        </div>
        <button 
          type="button"
          onClick={onMoreClick}
      ...
      <div className="flex-1 flex flex-col justify-between">
        {mandates.length === 0 ? (
          ...
          mandates.map((m) => (
            <div key={m.id} className="flex items-center justify-between group cursor-pointer py-2">
              <div className="flex items-center gap-2">
                <span className="text-[#1F2937] text-sm font-medium">{m.name}</span>
                <span className="text-[#1F2937] text-[10px] opacity-40" aria-hidden="true">[v]</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[#1F2937] text-xs font-medium">{m.status}</span>
                <div 

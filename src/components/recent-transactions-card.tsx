    ...
    <div className="bg-white border border-[#E4ECE9] border-dashed rounded-xl p-8 shadow-sm h-full flex flex-col">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-1.5">
          <h3 className="text-sm font-bold text-[#475569]">Recent Transactions</h3>
        ...
        {transactions.length === 0 ? (
          ...
          transactions.map((tx) => (
            <div key={tx.id} className="grid grid-cols-3 items-center py-2">
              {/* Left Column */}
              <span className="text-[#1F2937] text-sm font-medium">{tx.vendor}</span>
              
              {/* Center Column */}
              <span className="text-[#1F2937] text-sm font-bold text-center">{tx.amount}</span>
              
              {/* Right Column */}
              <div className="flex items-center gap-2 justify-end">
                <span className="text-[#1F2937] text-xs font-medium">{tx.status}</span>
                <div 
